...
int main() {
    ...
    while (window.isOpen()) {
        ...
        ball.setPosition(Vector2f(ball.getPosition().x + speedX, ball.getPosition().y + speedY));
        if (intersects(topWall, ball) || intersects(floor, ball)) {
            speedY *= -1;
        }
        if (intersects(rightWall, ball) || intersects(leftWall, ball)) {
            speedX *= -1;
        }
        window.clear();
        window.draw(text);
